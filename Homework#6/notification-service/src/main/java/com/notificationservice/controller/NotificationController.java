...
public class NotificationController {
    ...
    @Operation(summary = "Sent message about created user")
    @PostMapping(path = "/send-create-message", consumes = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> sendCreateMessage(
            @Parameter(description = "User's email", example = "mail@smail.su") @RequestBody String email) {
    ...
    @Operation(summary = "Sent message about deleted user")
    @PostMapping(path = "/send-delete-message", consumes = MediaType.APPLICATION_JSON_VALUE)
    public ResponseEntity<?> sendDeleteMessage(
            @Parameter(description = "User's email", example = "mail@smail.su") @RequestBody String email) {
